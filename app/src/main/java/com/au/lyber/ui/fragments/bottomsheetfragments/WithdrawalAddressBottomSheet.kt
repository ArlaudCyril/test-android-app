...
import com.au.lyber.databinding.BottomsheetWithdrawalAddressesBinding
                ...
                ORDINARY_VIEW -> {
                    itemList[position]?.let {
                        (holder as AddressViewHolder).binding.apply {
                            val data = it
                            val assest = BaseActivity.assets.firstNotNullOfOrNull{ item -> item.takeIf {item.id == data.network}}

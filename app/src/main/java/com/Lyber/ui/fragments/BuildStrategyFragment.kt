    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (isEdit){
            ...
            for (ada in strategy!!.bundle){
                val priceServiceResume = com.Lyber.ui.activities.BaseActivity.balanceResume.firstNotNullOfOrNull { item -> item.takeIf { item.id == ada.asset } }
...
//                        ContextCompat.getDrawable(requireContext(), R.drawable.button_purple_500)
    ...
    private fun clickListener(position: Int) {
        viewModel.addedAsset[position].let {
            ...
            val assest = com.Lyber.ui.activities.BaseActivity.assets.firstNotNullOfOrNull{ item -> item.takeIf {item.id ==viewModel.addedAsset[position].addAsset.id}}
